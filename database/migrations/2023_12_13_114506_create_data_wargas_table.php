   ...
   public function up(): void
{
    Schema::create('data_wargas', function (Blueprint $table) {
        $table->string('nik', 20,false, false)->unique()->primary(); //unique
        $table->string('nama', 200)->nullable(false);
        $table->enum('jns_kelamin', ['laki-laki', 'perempuan'])->nullable(false);
        $table->string('tempat_lahir',30,false)->nullable(false);
        $table->date('tgl_lahir')->nullable(false);
        $table->string('ayah',100,false)->nullable(false);
        $table->string('ibu',100,false)->nullable(false);
